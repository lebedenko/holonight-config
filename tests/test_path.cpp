...
TEST_CASE(xdg_path_precedes_home_fallback) {
  const auto result = resolveAppearancePath(Environment{{"XDG_CONFIG_HOME", "/xdg"}, {"HOME", "/home"}});
  EXPECT_TRUE(result);
  EXPECT_EQ(*result.value, std::filesystem::path{"/xdg/holonight/appearance.toml"});
}

TEST_CASE(home_fallback_and_empty_values_are_deterministic) {
  const auto result = resolveAppearancePath(
      Environment{{"HOLONIGHT_APPEARANCE_FILE", ""}, {"XDG_CONFIG_HOME", ""}, {"HOME", "/home/test"}});
  EXPECT_TRUE(result);
  EXPECT_EQ(*result.value, std::filesystem::path{"/home/test/.config/holonight/appearance.toml"});
}
