...
resolveAppearancePath(const Environment &environment) {
  if (std::string override_path =
          valueFor(environment, "HOLONIGHT_APPEARANCE_FILE");
      !override_path.empty()) {
    return Result<std::filesystem::path>::success(
        std::filesystem::path{std::move(override_path)});
  }
  if (std::string xdg = valueFor(environment, "XDG_CONFIG_HOME");
      !xdg.empty()) {
    return Result<std::filesystem::path>::success(
        std::filesystem::path{std::move(xdg)} / "holonight" /
        "appearance.toml");
  }
  if (std::string home = valueFor(environment, "HOME"); !home.empty()) {
    return Result<std::filesystem::path>::success(
        std::filesystem::path{std::move(home)} / ".config" / "holonight" /
        "appearance.toml");
  }
  return Result<std::filesystem::path>::failure({Diagnostic{
...
Result<std::filesystem::path> resolveAppearancePath() {
  return resolveAppearancePath(processEnvironment());
}
