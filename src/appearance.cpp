...
void validateIdentifier(std::vector<Diagnostic>& diagnostics, std::string_view field, const std::string& value,
  ...
  if (value.empty()) {
    ...
    addError(diagnostics, std::string{field} + " exceeds " + std::to_string(maximum_bytes) + " bytes");
  }
}

void validateFontSize(std::vector<Diagnostic>& diagnostics, std::string_view field, std::int64_t value) {
  if (value < kMinimumFontSize || value > kMaximumFontSize) {
    addError(diagnostics, std::string{field} + " must be in [6, 48]");
  }
}

void validateFiniteRange(std::vector<Diagnostic>& diagnostics, std::string_view field, double value, double minimum,
                         double maximum) {
  if (!std::isfinite(value) || value < minimum || value > maximum) {
    addError(diagnostics, std::string{field} + " is outside its accepted range");
  }
}
...
Appearance normalized(Appearance appearance) {
  ...
  appearance.typography.ui_family = trim(std::move(appearance.typography.ui_family));
  appearance.typography.monospace_family = trim(std::move(appearance.typography.monospace_family));
  appearance.typography.title_family = trim(std::move(appearance.typography.title_family));
  appearance.typography.display_family = trim(std::move(appearance.typography.display_family));
...
std::vector<Diagnostic> validate(const Appearance& appearance) {
  ...
  validateIdentifier(diagnostics, "typography.ui_family", canonical.typography.ui_family, 256);
  validateIdentifier(diagnostics, "typography.monospace_family", canonical.typography.monospace_family, 256);
  validateIdentifier(diagnostics, "typography.title_family", canonical.typography.title_family, 256);
  validateIdentifier(diagnostics, "typography.display_family", canonical.typography.display_family, 256);
  validateFontSize(diagnostics, "typography.ui_size", canonical.typography.ui_size);
  validateFontSize(diagnostics, "typography.monospace_size", canonical.typography.monospace_size);
  validateFontSize(diagnostics, "typography.title_size", canonical.typography.title_size);
  validateFontSize(diagnostics, "typography.display_size", canonical.typography.display_size);
  validateIdentifier(diagnostics, "icons.theme", canonical.icons.theme, 256);
  validateIdentifier(diagnostics, "icons.fallback", canonical.icons.fallback, 256);
  validateIdentifier(diagnostics, "icons.cursor", canonical.icons.cursor, 256);
  validateFiniteRange(diagnostics, "layout.scale", canonical.layout.scale, kMinimumLayoutScale, kMaximumLayoutScale);
  validateFiniteRange(diagnostics, "shape.scale", canonical.shape.scale, kMinimumShapeScale, kMaximumShapeScale);
  if (canonical.shape.base_radius.has_value()) {
    validateFiniteRange(diagnostics, "shape.base_radius", *canonical.shape.base_radius, 0.0, kMaximumShapeExtent);
  }
  if (canonical.shape.base_chamfer.has_value()) {
    validateFiniteRange(diagnostics, "shape.base_chamfer", *canonical.shape.base_chamfer, 0.0, kMaximumShapeExtent);
  }
  return diagnostics;
}
