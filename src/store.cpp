...
std::string systemMessage(std::string_view operation, int error_number) {
  return std::string{operation} + ": " + std::error_code{error_number, std::generic_category()}.message();
}
...
class TemporaryFile {
 public:
  ...
  [[nodiscard]] bool create(const std::filesystem::path& destination, int* error_number) {
    const std::filesystem::path directory = destination.parent_path().empty() ? "." : destination.parent_path();
    std::string pattern = (directory / ("." + destination.filename().string() + ".tmp-XXXXXX")).string();
...
Result<LoadedAppearance> load(const std::filesystem::path& path) {
  ...
  if (size > kMaximumDocumentBytes) {
    return Result<LoadedAppearance>::failure(
        {fileDiagnostic(ErrorCode::TooLarge, "appearance document exceeds 65536 bytes", path)});
  }

  std::ifstream input{path, std::ios::binary};
  if (!input) {
    return Result<LoadedAppearance>::failure(
        {fileDiagnostic(ErrorCode::IoError, "failed to open appearance file", path)});
  }
  std::string document;
...
Result<std::filesystem::path> writeAtomically(const Appearance& appearance, const std::filesystem::path& path) {
  if (path.empty() || path.filename().empty()) {
    return Result<std::filesystem::path>::failure(
        {fileDiagnostic(ErrorCode::AtomicWriteError, "appearance destination path is empty", path)});
  }
